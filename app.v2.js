...
const lab=$('[data-lab]');
if(lab){
 const state={scenario:'knowledge',severity:2,variability:3,latency:2,autonomy:2};
 ...
 $('[data-reset]',lab).addEventListener('click',()=>{$$('[data-scenario]',lab).forEach(x=>{const on=x.dataset.scenario==='knowledge';x.classList.toggle('active',on);x.setAttribute('aria-selected',String(on))});state.scenario='knowledge';Object.entries(presets.knowledge).forEach(([k,v])=>$(`#${k}`,lab).value=v);render()});
 render();
}
